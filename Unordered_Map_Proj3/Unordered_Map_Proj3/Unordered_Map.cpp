...
LRESULT CALLBACK WindowProcedure(HWND hWnd, UINT msg, WPARAM wp, LPARAM lp) {
	unordered_map<int, vector<string>> static map1;
	int static count = 0;

	if (count == 0) {
		map1 = readFile("C:\\Users\\tyler\\Documents\\SCHOOL\\COP\\100000 Sales Records.csv");
		count++;
	}
	switch (msg) {
	case WM_COMMAND:
		...
		case ENTER_BUTTON:
			...
			for (; iter != map1.end(); iter++) {
				if (iter->first == stoi(stringID)) {
					resultVec = iter->second;
					break;
				}
			}
			if (iter == map1.end()) {
				...
				region = resultVec[0];
				country = resultVec[1];
				unitsSold = resultVec[8];
				unitPrice = resultVec[9];
				unitCost = resultVec[10];
				totalRevenue = resultVec[11];
				totalCost = resultVec[12];
				totalProfit = resultVec[13];
...
unordered_map<int, vector<string>> readFile(string filename)
{
	...
	if (database.is_open())
	{
		...
		while (!database.eof())
		{
			...
			getline(database, temp, '\n');
			row.push_back(temp);
			map.insert(pair<int, vector<string>>(id, row));
			if (map.size() == 100000) {
				return map;
			}
		}
